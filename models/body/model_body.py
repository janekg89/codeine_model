# coding=utf-8 -*-
"""
PKPD model for whole-body codeine metabolism
"""
...
liver_id = 'codeine_liver'
kidney_id = 'codeine_kidney'

externalModelDefinitions = [
    mcomp.ExternalModelDefinition(sid="liver", source=f"../models/{liver_id}.xml", modelRef=liver_id),
    mcomp.ExternalModelDefinition(sid="kidney", source=f"../models/{kidney_id}.xml", modelRef=kidney_id),
]
...
SUBSTANCE_PATH = "body/data.xlsx"
SHEET_NAME = "substance"
substances = pd.read_excel(SUBSTANCE_PATH, sheet_name=SHEET_NAME, skiprows=[0], index_col="id", comment="#")
SUBSTANCES_BODY = substances.to_dict("index")
...
# replace species
replaced_species = {
    "li": substances.index.tolist(),
    "ki": substances.index.tolist(),
...
for skey in substances.index.tolist():
    replacedElements.append(
...
def create_model(target_dir):
    return creator.create_model(
        modules=['models.body.model_body'],
